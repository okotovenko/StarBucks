(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".events__slider-1")) new core(".events__slider-1", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 3,
    ...
    class ScrollWatcher {
        ...
        scrollWatcherCallback(entry, observer) {
            ...
            if ("star" === targetElement.dataset.watch && targetElement.classList.contains("_watcher-view")) {
                let animElement = document.querySelectorAll(`._watcher-view.star`);
                if (animElement.length > 0) animElement.forEach((element => {
                    element.classList.add("animation");
                }));
            }
            if ("cup" === targetElement.dataset.watch && targetElement.classList.contains("_watcher-view")) {
                let animElement = document.querySelectorAll(`._watcher-view.new__cup-ibg`);
                if (animElement.length > 0) animElement.forEach((element => {
                    element.classList.add("animation");
                }));
            }
            if ("counter" === targetElement.dataset.watch && targetElement.classList.contains("_watcher-view")) {
                let animElement = document.querySelectorAll(`._watcher-view.new__text, ._watcher-view.we__item, \n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t._watcher-view.new__button.button-green, ._watcher-view.new__button.button-dark,\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t._watcher-view.delicious__text, ._watcher-view.item-counters,\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t._watcher-view.products__text, ._watcher-view.events__text,\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t._watcher-view._icon-instagram, ._watcher-view._icon-phone`);
                if (animElement.length > 0) animElement.forEach((element => {
                    element.classList.add("animation");
                }));
            }
            if ("title" === targetElement.dataset.watch && targetElement.classList.contains("_watcher-view")) {
                let animElement = document.querySelectorAll("._watcher-view.title");
                if (animElement.length > 0) animElement.forEach((element => {
                    element.classList.add("animation");
                }));
            }
        ...
        window.addEventListener("load", (function() {
            ...
            titleArray.forEach((element => {
                ...
                if (!element.childNodes[1].classList.contains("title-span")) titleArray.forEach((element => {
                    ...
                    for (let i = 0, time = 0; i < element.children.length; i++, time += .1) element.children[i].style = `\n\t\t\t\t\tanimation-delay: ${time}s;\n\t\t\t\t\t\t`;
                })); else {
    ...
    function cursorMove() {
        ...
        const animateMe = function(e) {
            ...
            let move = 10;
            let xMove = x / width * (2 * move) - move;
            let yMove = y / height * (2 * move) - move;
            span.style.transform = `translate(${xMove}px, ${yMove}px)`;
            if ("mouseleave" === e.type) span.style.transform = "";
        };
        const editCursor = e => {
    ...
    function deleteTextSpanVideo() {
        window.addEventListener("resize", (function() {
            const windowInnerWidth = window.innerWidth;
            const spanVideoPlay = this.document.querySelector("span._icon-triangle");
            if (windowInnerWidth < 401) spanVideoPlay.innerText = ""; else spanVideoPlay.innerText = "Cooking Process";
        }));
    }
    function preloader() {
        window.addEventListener("load", (function() {
